...
pub enum Role { Admin, Verifier, Operator, Auditor }

#[contracttype]
#[derive(Clone)]
pub struct RoleAssignment { pub address: Address, pub role: Role, pub assigned_at: u64, pub assigned_by: Address }
...
impl RoleManager {
    ...
    pub fn assign_role(env: Env, admin: Address, target: Address, role: Role) {
        ...
        env.storage().persistent().set(&role_key, &true);
        env.events().publish((Symbol::short("role_assigned"),), (target, role));
    }
    ...
    pub fn revoke_role(env: Env, admin: Address, target: Address, role: Role) {
        ...
        env.events().publish((Symbol::short("role_revoked"),), (target, role));
    }

    pub fn has_role(env: Env, address: Address, role: Role) -> bool {
        env.storage().persistent().get(&(role, address)).unwrap_or(false)
    }

    fn require_role(env: &Env, address: &Address, role: Role) {
        if !env.storage().persistent().get(&(role, address.clone())).unwrap_or(false) {

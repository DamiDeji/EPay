...
impl InvoiceManager {
    ...
    pub fn issue_invoice(env: Env, invoice_id: u64) {
        ...
        env.storage().persistent().set(&invoice_id, &invoice);
        env.events()
            .publish((Symbol::new(&env, "invoice_issued"),), (invoice_id,));
    }
    ...
    pub fn get_invoice(env: Env, invoice_id: u64) -> Option<InvoiceData> {
        env.storage().persistent().get(&invoice_id)
    }
    pub fn invoice_exists(env: Env, invoice_id: u64) -> bool {
        env.storage().persistent().has(&invoice_id)
    }
}

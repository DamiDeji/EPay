...
impl SubscriptionManager {
    ...
    pub fn create_subscription(
        ...
        let sub = SubscriptionData {
            ...
            payments_made: 0,
            max_payments,
            next_billing: now + 86400 * 30,
            created_at: now,
            cancelled_at: None,
        };
        env.storage().persistent().set(&sub_id, &sub);
        env.events()
            .publish((Symbol::new(&env, "sub_created"),), (sub_id,));
        sub_id
    }

    pub fn renew(env: Env, sub_id: u64) {
        let mut sub: SubscriptionData = env
            .storage()
            .persistent()
            .get(&sub_id)
            .unwrap_or_else(|| panic!("Subscription not found"));
        sub.payments_made += 1;
        if let Some(max) = sub.max_payments {
            if sub.payments_made >= max {
        ...
        env.storage().persistent().set(&sub_id, &sub);
        env.events()
            .publish((Symbol::new(&env, "sub_renewed"),), (sub_id,));
    }
    ...
    pub fn pause(env: Env, sub_id: u64) {
        ...
        env.storage().persistent().set(&sub_id, &sub);
        env.events()
            .publish((Symbol::new(&env, "sub_paused"),), (sub_id,));
    }
    ...
    pub fn cancel(env: Env, sub_id: u64) {
        ...
        env.storage().persistent().set(&sub_id, &sub);
        env.events()
            .publish((Symbol::new(&env, "sub_cancelled"),), (sub_id,));
    }

    pub fn get_subscription(env: Env, sub_id: u64) -> Option<SubscriptionData> {
        env.storage().persistent().get(&sub_id)
    }
    pub fn subscription_exists(env: Env, sub_id: u64) -> bool {

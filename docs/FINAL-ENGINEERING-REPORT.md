...
| P2       | ~~Container scanning (Trivy) and per-image SBOM.~~ The steps existed but pinned `trivy-action@0.29.0`, a tag that does not exist, so the job failed before scanning anything. Now on `v0.36.0`.                                                                                   | **Done 2026-09-15**                      |
| P3       | ~~Image signing / provenance for releases.~~ cosign keyless was configured but only runs on release tags, and none had ever been created because semantic-release could not load its commit preset. Preset package now installed into the dlx environment.                        | **Done 2026-09-15**                      |
| P2       | ~~The Release workflow never produced a tag or GitHub Release.~~ `pnpm dlx semantic-release` could not resolve the `conventionalcommits` preset.                                                                                                                                  | **Done 2026-09-15**                      |
| P2       | ~~The Helm drift check failed in CI.~~ Committed manifests were generated by Helm 4 while CI pins Helm 3.16.3; the render script now normalises the one whitespace difference.                                                                                                    | **Done 2026-09-15**                      |
| P2       | ~~`@epay/database#typecheck` failed intermittently.~~ `prisma/seed.ts` imported the package by its own name, so the check required a concurrent build.                                                                                                                            | **Done 2026-09-15**                      |
| P1       | **The nightly database backup has never succeeded** — `AWS_REGION`, `BACKUP_AWS_ROLE_ARN`, `PRODUCTION_DATABASE_URL` and `BACKUP_BUCKET` are unset in this repository, so `configure-aws-credentials` fails with "Input required and not supplied: aws-region". No backup exists. | Open — needs repository secrets          |
...
| 11  | DevOps               | **PARTIAL** | Helm lint/template/digest/drift pass and are Helm-major agnostic; Docker builds | Image scan/signing now unblocked but unproven until a release runs |

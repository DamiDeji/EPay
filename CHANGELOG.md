...
- The CI Helm job failed on every push. The committed `k8s/manifests.yaml` was
  generated by Helm 4, while CI pins Helm 3.16.3, and the two differ by a blank
  line before each document separator — so the drift check compared two different
  renderers. `scripts/render-k8s-manifests.sh` now normalises that whitespace,
  and the committed file was regenerated with it; the check passes with either
  major.
- `@epay/database#typecheck` failed intermittently on a clean checkout.
  `prisma/seed.ts` imported the package by its own name, which resolves to
  `dist/index.d.ts`, while turbo's `typecheck` depends only on _dependencies'_
  builds — so the check raced its own `build` and lost (`TS2307`). The seed now
  imports the package's source, and the check passes with `dist/` absent.
- The Trivy steps in `supply-chain.yml` pinned
  `aquasecurity/trivy-action@0.29.0`, a tag that does not exist (the project tags
  its releases `v0.30.0` and up), so the job failed before scanning anything.
  Both steps are now on `v0.36.0`.
- The Release workflow never produced a tag or a GitHub Release:
  `pnpm dlx semantic-release` could not resolve the `conventionalcommits` preset
  that `.releaserc.json` requires, because the preset is not a dependency of
  semantic-release itself. It is now installed into the same temporary
  environment with `--package`.

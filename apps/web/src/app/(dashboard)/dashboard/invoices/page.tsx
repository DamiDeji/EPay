...
const statusBadges: Record<string, 'success' | 'warning' | 'destructive' | 'slate'> = {
  PAID: 'success',
  SENT: 'slate',
  OVERDUE: 'destructive',
  DRAFT: 'warning',
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          { label: 'Total Invoices', value: '24', sub: '6 pending' },
          { label: 'Total Value', value: '3,450 TON', sub: '$8,970 USD' },
          { label: 'Paid', value: '18', sub: '75% rate' },
          { label: 'Overdue', value: '2', sub: '8.3% rate' },
        ].map((stat) => (
          <Card key={stat.label}>
                    ...
                    >
                      <td className="py-4 px-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-mono text-xs font-medium text-[#0098EA]">{invoice.id}</span>
                        </div>
                      </td>
                      <td className="py-4 px-4 font-medium text-slate-900 dark:text-white">{invoice.customer}</td>
                      <td className="py-4 px-4 font-semibold text-slate-900 dark:text-white">{invoice.amount}</td>
                      <td className="py-4 px-4">
                        <Badge variant={statusBadges[invoice.status]} className="gap-1">
